...
const BADGE = '[![¿Cuántos sitios están vivos?](https://raw.githubusercontent.com/indi-es/estudios/reachable-sites/reachable-site.svg)](https://github.com/indi-es/estudios/blob/reachable-sites/reachable-site-errors.txt)'
const DIVIDER = {p: '---'};
...
async function createMD(sections) {
  ...
  const jsonStudios = sections.map((section) => [
    { h2: section.title },
    {
      ul: section.data.Estudios.map((item) => ({
